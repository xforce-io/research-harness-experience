...
* **Call Chain 就是天然的 Action-Component Graph**：
  - KWeaver 的 TraceAI 已经记录了 Action 之间的调用关系（Call Chain）
  - 可以直接在 Call Chain 数据上运行 AgentSeer 的拓扑分析
* **安全审计用例**：
  - 在 TraceAI 的 Triage Center 中增加"拓扑异常检测"维度
  - 不仅检测"失败"，还检测"成功但路径异常"的轨迹（潜在安全隐患）
* **待确认**：
  - [ ] KWeaver 的 Call Chain 粒度是否足够细，能否区分不同的工具调用组合？
...
- [ ] 在 KWeaver 的 DPH 编排场景下，"预期拓扑"可以从 DPH 编排定义中自动推导，用作 baseline
